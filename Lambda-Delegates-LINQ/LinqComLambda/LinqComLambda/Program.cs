...
namespace LinqComLambda
{
    class Program
    {
        ...
        static void Main(string[] args)
        {
            ...
            var r14 = products.Where(p => p.Category.Id == 5).Select(p => p.Price).DefaultIfEmpty(0.0).Average(); //"DefaultIfEmpty - se o resultado for uma colecao vazia, atribui 0.0"
            Console.WriteLine("Category '5' Average prices: " + r14);

            //operacoes agregadas personalizadas Map e Reduce - Em c# Select e Aggregate
            var r15 = products.Where(p => p.Category.Id == 1).Select(p => p.Price).Aggregate(0.0, (x, y) => x + y); //0.0 = valor padrao inicial (para nao dar falha caso a colecao esteja vazia)
            ...
            foreach(IGrouping<Category, Product> group in r16) //percorrendo cada elemento do resultado
            {
